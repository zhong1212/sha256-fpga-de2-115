// File: SHA256.v
// Module SHA256 hoàn chỉnh (Full SHA-256 Core Engine)
// Tên module: SHA256 (Tương thích hoàn toàn với Quartus Prime)

module SHA256 (
    input  wire         clk,
    input  wire         reset,
    input  wire         start,       // Tín hiệu kích hoạt băm (Xung mức cao '1')
    input  wire [511:0] block_in,    // Khối dữ liệu đầu vào 512-bit
    output reg  [255:0] digest,      // Kết quả Hash 256-bit đầu ra
    output wire         ready,       // Báo sẵn sàng nhận dữ liệu mới
    output reg          done         // Báo đã băm xong
);

    // =========================================================================
    // 1. KHAI BÁO CÁC HÀM LOGIC CỐT LÕI (SHA-256 FUNCTIONS)
    // =========================================================================
    ...
    function [31:0] sig0 (input [31:0] x);
        sig0 = {x[6:0], x[31:7]} ^ {x[17:0], x[31:18]} ^ {3'b000, x[31:3]};
    endfunction

    function [31:0] sig1 (input [31:0] x);
        sig1 = {x[16:0], x[31:17]} ^ {x[18:0], x[31:19]} ^ {10'b000, x[31:10]};
    endfunction

    // =========================================================================
    // 2. LỊCH THÔNG ĐIỆP W_t (MESSAGE SCHEDULER)
    // =========================================================================
    reg [31:0] w [0:63];
    integer i;
    ...
    // =========================================================================
    // 3. BẢNG HẰNG SỐ K_t (64 VALUES) - ĐÃ LÀM SẠCH LỖI
    // =========================================================================
    reg [31:0] k [0:63];
    initial begin
    ...
    // =========================================================================
    // 4. MÁY TRẠNG THÁI FSM VÀ ĐƯỜNG DỮ LIỆU BĂM (DATAPATH)
    // =========================================================================
    ...
    // 8 Thanh ghi trạng thái A-H
    reg [31:0] a, b, c, d, e, f, g, h;
    
    // Giá trị khởi tạo H0->H7 chuẩn SHA-256
        ...
        if (reset) begin
            ...
            case (state)
                STATE_IDLE: begin
                    ...
                    if (start) begin
                        // Nạp và Mở rộng dữ liệu W_0..W_63
                ...
                STATE_HASH: begin
                    // Cập nhật 8 thanh ghi qua từng nhịp clock
                    h <= g;
                    g <= f;
                    f <= e;
                    e <= d + t1;
                ...
                STATE_DONE: begin
                    // Cộng dồn kết quả với H0..H7 ban đầu
