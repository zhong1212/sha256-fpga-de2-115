...
module udp_rx_parser (
    ...
    input  wire        rx_dv,
    output reg         mac_ip_match,     // Đại diện cho packet_valid ở Top
    output reg  [15:0] packet_length,
    output wire [3:0]  fsm_state_out,
    output reg         raw_packet_seen,
    output wire        ip_matched_out,   // Đã bổ sung
    output wire        port_matched_out  // Đã bổ sung
);
    ...
    // Gán tín hiệu ra ngoài cho file Top
    assign ip_matched_out   = match_ip_norm | match_ip_swap;
    assign port_matched_out = match_port_norm | match_port_swap;
    assign fsm_state_out    = {match_spark, ip_matched_out, port_matched_out, 1'b0};
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            ...
            if (!rx_dv) begin
                // SỬA LỖI LOGIC TẠI ĐÂY: Chỉ Pass khi thỏa mãn TẤT CẢ điều kiện
                if ((len_cnt > 16'd14) && ip_matched_out && port_matched_out && match_spark) begin 
                    mac_ip_match  <= 1'b1;
